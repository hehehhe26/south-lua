...
contextActionService:BindActionAtPriority("ReleaseGUIFocus", function(actionName, inputState, inputObject)
    if inputState == Enum.UserInputState.Begin then
        pcall(function() guiService.SelectedCore = nil end)
    end
    return Enum.ContextActionResult.Pass
end, false, Enum.KeyCode.E, Enum.KeyCode.W, Enum.KeyCode.A, Enum.KeyCode.S, Enum.KeyCode.D)

print("✅ Script loaded - Adding all original features...")
...
local farmPositions = {
    {
        ...
        getPrompt = function()
            local success, result = pcall(function()
                return workspace:FindFirstChild("ButchersJob") and 
                       workspace.ButchersJob:FindFirstChild("Step2") and 
                       workspace.ButchersJob.Step2:GetChildren()[2] and 
                       workspace.ButchersJob.Step2:GetChildren()[2]:FindFirstChild("Carcass") and 
                       workspace.ButchersJob.Step2:GetChildren()[2].Carcass:FindFirstChild("Prompt")
            end)
    ...
    },
    {
        ...
        getPrompt = function()
            local success, result = pcall(function()
                return workspace:FindFirstChild("ButchersJob") and 
                       workspace.ButchersJob:FindFirstChild("Step3") and 
                       workspace.ButchersJob.Step3:FindFirstChild("PlaceChopMeat") and 
                       workspace.ButchersJob.Step3.PlaceChopMeat:FindFirstChild("Meat") and 
                       workspace.ButchersJob.Step3.PlaceChopMeat.Meat:FindFirstChild("Prompt")
            end)
            return success and result or nil
        end,
        name = "Chop",
    ...
    },
    {
        ...
        getPrompt = function()
            local success, result = pcall(function()
                return workspace:FindFirstChild("ButchersJob") and 
                       workspace.ButchersJob:FindFirstChild("SellMeat") and 
                       workspace.ButchersJob.SellMeat:FindFirstChild("MeatSell") and 
                       workspace.ButchersJob.SellMeat.MeatSell:FindFirstChild("Prompt")
            end)
            return success and result or nil
        end,
        name = "Sell",
...
-- Location names
local locationNames = {"Carcass", "Chop", "Sell", "Butcher", "Meat"}

-- State variables
local isFarming = false
local antiPlayerEnabled = true
local currentStep = 1
local tween = nil
local showESP = false
...
local function clearESP()
    for _, child in pairs(billboardHolder:GetChildren()) do
        child:Destroy()
    end
    espObjects = {}
    npcBillboards = {}
...
-- Create ESP
local function createESP(entity, name)
    local highlight = Instance.new("Highlight")
    highlight.Name = "EntityESP"
    highlight.FillColor = Color3.fromRGB(230, 90, 90)
    highlight.OutlineColor = Color3.fromRGB(255, 255, 255)
    highlight.FillTransparency = 0.3
    highlight.OutlineTransparency = 0.5
    highlight.Adornee = entity
    highlight.Parent = billboardHolder
    ...
    billboardFrame.BackgroundColor3 = Color3.fromRGB(18, 20, 26)
    ...
    billboardCorner.CornerRadius = UDim.new(0, 6)
    ...
    billboardText.Text = "🔫 " .. name
    billboardText.TextColor3 = Color3.fromRGB(230, 120, 120)
    billboardText.Font =
